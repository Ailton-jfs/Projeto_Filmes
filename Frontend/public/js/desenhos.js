    ...
    async function carregarDesenhos() {
      ...
      try {
        const resposta = await fetch("http://localhost:3000/api/recomendacoes", {
          ...
          body: JSON.stringify({ preferencias: "Desenhos e animações populares" })
        });
        ...
        desenhos.forEach(desenho => {
          ...
          card.innerHTML = `
            <img src="${desenho.poster}" alt="${desenho.titulo}" class="rounded-xl shadow-lg object-cover w-full h-[300px]" />
            <div class="absolute inset-0 bg-black/70 flex items-center justify-center opacity-0 group-hover:opacity-100 transition">
              <p class="text-white text-center px-2 font-semibold">${desenho.titulo}</p>
            </div>
          `;
          card.addEventListener("click", () => window.open(desenho.link_tmdb, "_blank"));
